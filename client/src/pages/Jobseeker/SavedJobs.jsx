...
const SavedJobs = () => {
  ...
  const validSavedJobs = savedJobList.filter((s) => s?.job);
  ...
  const handleUnsaveJob = async (jobId) => {
    try {
      axiosInstance.delete(API_PATHS.JOBS.UNSAVE_JOB(jobId));
      toast.success("Job removed successfully!");
      getSavedJobs();
    } catch (error) {
      console.error("Error is deleting job from saved list: ", error);
              ...
              {validSavedJobs.length === 0 ? (
                <div className="text-center py-16 lg:py-20 bg-white/60 backdrop-blur-xl rounded-2xl border border-white/20">
                  ...
                  <p className="text-gray-600 mb-8">
                    Start saving jobs that interests you to view them later
                  </p>
                  <button
