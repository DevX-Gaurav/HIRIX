...
const JobCard = ({ job, onClick, onToggleSave, onApply, saved, hideApply }) => {
  ...
  const formatSalary = (min, max) => {
    ...
    return `${formatNumber(min)}/PA`;
  };

  return (
    <section
      onClick={onClick}
      className=" p-6 bg-white rounded-2xl border border-gray-200 hover:shadow-xl hover:shadow-gray-200 transition-all duration-300 group relative overflow-hidden cursor-pointer"
    >
          ...
          >
            <Bookmark
              className={`w-5 h-5 hover:text-blue-600 ${
                job?.isSaved || saved ? "teext-blue-600" : "text-gray-400"
              }`}
            />
          </button>
        )}
      ...
      <div className="flex items-center justify-between font-medium text-gray-500 mb-5 pb-4 border-b border-gray-100 text-xs">
